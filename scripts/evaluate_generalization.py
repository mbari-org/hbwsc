...
def main():
    ...
    parser.add_argument(
        "--max-noise", type=float, default=40.0, help="Maximum allowed noise percentage (default: 40.0)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional: limit to top N configs by DBCV (default: run all passing filters)",
    )
    parser.add_argument(
        "--allow-no-overlap",
        action="store_true",
        help="Include configs where window_sec == hop_sec (dropped by default)",
    )
    parser.add_argument("--drop-noise", action="store_true", help="Drop noise points when training classifier")
    parser.add_argument(
        "--eval-audio", type=Path, default=None, help="Auto-initialize eval session with this audio file"
    )
    parser.add_argument(
        "--eval-labels", type=Path, default=None, help="Auto-initialize eval session with these Raven labels"
    )
    parser.add_argument(
        "--eval-embeddings-dir",
        type=Path,
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            if not args.allow_no_overlap and float(row["window_sec"]) == float(row["hop_sec"]):
                continue
            if (
                args.min_clusters <= n_clusters <= args.max_clusters
                and noise_pct <= args.max_noise
                and dbcv >= args.min_dbcv
            ):
    ...
    if not valid_runs:
        print(
            f"No configurations matched the criteria: {args.min_clusters} <= n_clusters <= {args.max_clusters} AND noise <= {args.max_noise}% AND dbcv >= {args.min_dbcv}"
        )
        sys.exit(1)
    ...
    for i, r in enumerate(top_runs):
        ...
        print(
            f" {i + 1}. {prefix}{r['umap_dims']}D, mcs={r['mcs']}, eps={r['eps']} | n={r['n_clusters']}, noise={r['noise_pct']}%, dbcv={r['dbcv']}"
        )
    ...
    for idx, run in enumerate(top_runs):
        session_dir = None
        if "hop_pct" in run:
            hop_pct_val = str(run["hop_pct"])
            if "." in hop_pct_val:
                hop_pct_val = hop_pct_val.split(".")[0]
            pct_name = f"win{run['window_sec']}_hop{hop_pct_val}pct_{run['embedder_type']}_{run['perch_padding']}"
            if (sweep_embed_dir / pct_name).exists():
                session_dir = sweep_embed_dir / pct_name
                sub_session_name = pct_name

        if session_dir is None:
            sub_session_name = (
                f"win{run['window_sec']}_hop{run['hop_sec']}_{run['embedder_type']}_{run['perch_padding']}"
            )
        ...
        print(f"\n{'=' * 70}")
        print(
            f"=== [{idx + 1}/{len(top_runs)}] {session_dir.name}: {run['umap_dims']}D, mcs={run['mcs']}, eps={run['eps']}"
        )
        print(f"{'=' * 70}")
    ...
    print(f"{'=' * 115}")
    print(
        f"{'Session':<30} | {'Win/Hop/Pad':<20} | {'Config':<20} | {'DetSim':<6} | {'NMI':<6} | {'ARI':<6} | {'Homog':<6} | {'Aggreg':<6}"
    )
    print("-" * 115)
    ...
    for res in master_results:
        ...
        print(
            f"{res['train_session']:<30} | {win_str:<20} | {res['config']:<20} | {res['DetSim']:<6.4f} | {res['NMI']:<6.4f} | {res['ARI']:<6.4f} | {res['Homogeneity']:<6.4f} | {res['Aggregated']:<6.4f}"
        )
