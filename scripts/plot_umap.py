...
if noise_mask.any():
    ax.scatter(
        ...
        c="lightgrey", s=8, linewidths=0, alpha=0.6, label="noise", zorder=1,
    )
...
for cid in cluster_ids:
    ...
    ax.scatter(
        xy[mask, 0], xy[mask, 1],
        c=[color], s=18, linewidths=0, alpha=0.85,
        label=f"cluster {cid} (n={mask.sum()})", zorder=2,
    )
    # Mark centroid
    cx, cy = xy[mask].mean(axis=0)
    ax.scatter(cx, cy, c=[color], s=120, marker="*", edgecolors="white",
